...
static func scan_path(
  ...
  if path.size() < 2:
    ...
    return {"reach": 0.0, "reach_point": only, "blocked": false, "path": path}
  ...
  for i in range(path.size() - 1):
    ...
    if clear_frac < 1.0 - 1e-6:
      ...
      return {"reach": traveled, "reach_point": reach_point, "blocked": true, "path": truncated}
    traveled += seg_len
  return {"reach": traveled, "reach_point": path[path.size() - 1], "blocked": false, "path": path}
