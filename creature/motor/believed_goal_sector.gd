...
## Unit step direction [param d] → sector index 0..7 (N, NE, E, SE, S, SW, W, NW; **+Y = N** in world space).
static func sector_index_for_step(d: Vector2) -> int:
  if d.length_squared() < 1e-8:
    return 0
  var u := d.normalized()
  var angle := atan2(u.x, -u.y)
  if angle < 0.0:
    angle += TAU
