...
static func is_backtrack_step(step_dir: Vector2, approach_dir: Vector2, min_dot: float) -> bool:
...
static func infer_approach_dir(
  ...
  if not trail_centers.is_empty():
    ...
    if typeof(anchor) == TYPE_VECTOR2:
      var delta := creature_pos - anchor as Vector2
...
static func record(io_state: Dictionary, approach_dir: Vector2, physics_tick: int, ttl_ticks: int) -> void:
...
static func active_dir(io_state: Dictionary, physics_tick: int) -> Vector2:
  if io_state.is_empty():
    return Vector2.ZERO
  if physics_tick >= int(io_state.get("until_tick", 0)):
    return Vector2.ZERO
  var d: Variant = io_state.get("dir", null)
  if typeof(d) == TYPE_VECTOR2 and (d as Vector2).length_squared() > 1e-12:
    return d as Vector2
  return Vector2.ZERO
