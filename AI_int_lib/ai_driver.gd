...
## GOAL-TARGET MEMORY (not implemented — design anchor for ENGINE + future LLM beliefs).
...
## Rough draft: per-round table keyed by target [code]instance_id[/code] — e.g. [code]food_plants[/code] bushes, prey bodies, mates, squeeze/nest cues — discriminator TBD alongside [code]SeekCandidate[/code] façade.
## - [b]Precise tier — stationary[/b]: while remembered and distance to last world pos lies within [code]goal_memory_precise_radius_px[/code] (baseline **1000** in merge comments), merge exact [code]Vector2[/code] into seek/unready pathways (today [code]food_seek_targets[/code] / analogous lists until unified).
## - [b]Precise tier — moving[/b]: small disk around last known position (radius [code]goal_memory_moving_last_known_radius_px[/code], clamp [code]≤ goal_memory_precise_radius_px[/code] unless waived) — see CREATURE_MOVEMENT_V2 §F.
## - [b]Coarse tier[/b]: beyond precise envelope but not forgotten — do **not** store a fixed world compass label; each tick recompute an **egocentric** 8-way bucket from [code]entry.world_pos - creature_pos[/code]: N, NE, E, SE, S, SW, W, NW (45° sectors, +Y = N in world space). Use for weak motor bias, jeopardy routing, or perception text only. Discard coarse-only entries after [code]goal_memory_coarse_ttl_sec[/code] continuously in coarse (CREATURE_MEMORY §5.3).
...
## **Live sense only today.** Goal-target memory (precise coords / mover disk per [code]goal_memory_*[/code], else egocentric 8-way) merges **after** this call inside [_build_motor_context] — see [_goal_belief] design block above.
...
func _build_motor_context(
  ...
  ## for coarse-only beliefs, expose [code]goal_memory_coarse_sectors[/code] (8-way strings) or a weak cardinal cost — do not append stale Vector2 beyond precise envelope.
