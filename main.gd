...
## Duel round cap (seconds) — CREATURE_GOALS v1.
const ROUND_DURATION_SEC := 10.0
...
func _ready() -> void:
	...
	_ensure_food_plants()
	if not has_node("RoundTimer"):
		var rt := Timer.new()
		rt.name = "RoundTimer"
		rt.one_shot = true
		rt.wait_time = ROUND_DURATION_SEC
		rt.timeout.connect(_on_round_timer_timeout)
		add_child(rt)
...
func _ensure_spawn_markers() -> void:
	...
	if get_node_or_null("HerbivoreSpawn") == null:
		...
		hs.position = Vector2(vp.x * 0.82, margin_y)
		add_child(hs)
...
func _herbivore_spawn_position() -> Vector2:
	...
	return Vector2(get_viewport().get_visible_rect().size.x * 0.82, get_viewport().get_visible_rect().size.y * 0.5)
...
func _spawn_duel_carnivore(pos: Vector2) -> void:
	...
	if mob == null:
		return
	mob.position = pos
	mob.linear_velocity = Vector2.RIGHT * 200.0
