...
description: >-
  ...
  godot --path . --headless -s res://tests/run_all.gd (or a caller-named subset).
...
- Run `godot --path . --headless -s res://tests/run_all.gd` or the focused runner named by the caller before declaring completion.
- Run `python tools/check_gdscript_no_tabs.py` when GDScript under test or touched production paths changed.
- Do not fix production bugs inside `tests/` — report the failing contract to the appropriate domain agent via your summary back to the caller.
