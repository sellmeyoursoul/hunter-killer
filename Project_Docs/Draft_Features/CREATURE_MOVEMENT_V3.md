...
- **Observation-scaled:** `prey_race_giveup_ticks` is recomputed per tick (`creature_motor_stack.gd::_refresh_prey_race_giveup_ticks`) from the predator's `stat_observation` via the same `CreatureStatCurve.saturating` curve and worst/best lerp shape as WAIT's composure discount (§6.1 concealment-rest addendum) — higher observation realizes a losing race sooner (`prey_race_giveup_ticks_best`, fewer ticks tolerated) than low observation (`_worst`, more ticks).
- **Config:** `prey_race_giveup_ticks_worst` (90) / `_best` (20), `prey_race_observation_curve_anchor_stat` (10.0) / `_anchor_value` (0.75), `prey_race_not_closing_epsilon` (0.05 — float-jitter slack before "improving" counts), `prey_race_exclusion_cooldown_ticks` (60).
