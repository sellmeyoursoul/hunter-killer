...
<<Comment (2026-09-24, pre-existing bug found in the decision 45 fix pass, NOT fixed, needs a user decision; see [PHYSICS_SQUEEZE.md decision 45 I](PHYSICS_SQUEEZE.md) and the High backlog row "Locale salient writes refused outside the +x/+z quadrant").
- **What the code does.** `GoalSourceMemoryStore.anchor_cell_in_bounds` (`goal_source_memory.gd`) implements the bounds rule above literally: `cx = floor(x / cell_px)` and `cy = floor(z / cell_px)` from world origin, and it rejects `cx < 0` or `cy < 0`. The live default env grid (`main_3d.gd` `_create_default_open_grid`) also has `origin_world = (0,0)`.
- **Why that breaks.** The live playfield is centred on the origin, spanning about (−100,−100) to (100,103.7). So every locale salient write with an anchor at x < 0 or z < 0 is silently refused. That covers `find_food` (food anchor) and `avoid_hostiles` (body anchor) alike, over roughly 3/4 of the field.
- **Secondary observation.** The upper bound compares a coverage-cell index (`cell_px` 52u) against the env grid's `cell_width`/`cell_height`, which count env cells (4u on the default grid). So it is far looser than intended.
- **Scope of the design flaw.** This table assumed a grid whose origin is its corner. No fix is decided: candidates include offsetting by the playfield/grid origin, or dropping the lower bound. Changing the rule changes this section's hash inputs.>>
...
| `flee_separation_gain` (`creature_motor_v3`) | **0.25** | **Shipped 2026-09-24 (decision 45 A).** Every flee candidate scores `+ flee_dist × gain × separation` (separation = clamped change in nearest-threat distance, endpoint vs now). Bounds: ≥ ~0.25 to fix the review repro; above ~0.33 the choke fit-gate test fails. Canonical: [PHYSICS_SQUEEZE.md decision 20](PHYSICS_SQUEEZE.md). |
