...
| **Origin** | **Locale identity** (`cell_x`, `cell_y`, `context_hash`): floor division from **world `Vector2.ZERO`**, **not** [`EnvironmentGridBaked.origin_world`](../../environment/environment_grid_baked.gd). Indices **may be negative** on the origin-centred playfield; that is valid (they are only hash inputs and row fields). The **bounds gate** (next row) is separate and does use the env grid's own origin. |
...
| **Out of bounds** | **Reject entire salient write** (no clamp-to-edge). When **`EnvironmentGridBaked`** is valid, map the anchor with the **env grid's own** `grid.world_to_cell(MotorPlane.to_grid_world(anchor))` (respects `origin_world` and the env grid's `cell_size`) and require the resulting env-cell index ∈ **`[0, cell_width) × [0, cell_height)`**. This is an env-grid footprint test in env cells, **not** a test on the 52u locale indices above. A grid with `cell_size <= 0` rejects. If env grid is missing/invalid at outcome, **reject** salient write. Code: `GoalSourceMemoryStore.anchor_cell_in_bounds` (`goal_source_memory.gd`). **Live grid:** `main_3d.gd` `_create_default_open_grid()` sets `origin_world = get_motor_playfield_bounds_min()` (since 2026-09-24), so the 4u grid covers about x ∈ [−100, 100), z ∈ [−100, 104) — the whole playfield. The upper edge is half-open: an anchor at exactly x = 100.0 is refused. |
...
~~<<Comment (2026-09-24): pre-existing bug — `anchor_cell_in_bounds` refused every locale salient write with an anchor at x < 0 or z < 0 (≈3/4 of the origin-centred playfield), and its upper bound compared 52u coverage-cell indices against 4u env-cell counts…>>~~ **Resolved 2026-09-24 (fixed; [PHYSICS_SQUEEZE.md decision 45 I](PHYSICS_SQUEEZE.md)):**
- **Root cause was upstream.** `main_3d.gd` `_create_default_open_grid()` built the only live `EnvironmentGridBaked` with `origin_world = Vector2.ZERO`, so the 4u grid covered only x ∈ [0, 200), z ∈ [0, 204) — the +x/+z quadrant of the (−100,−100)–(100,103.7) playfield. The grid now starts at `get_motor_playfield_bounds_min()`.
- **Bounds gate rewritten** to the env grid's own `world_to_cell` (the **Out of bounds** row above), which also removes the 52u-vs-4u unit mismatch.
- **Locale identity unchanged.** `cell_x`/`cell_y`/`context_hash` still floor from world origin (the **Origin** row above), so existing hash inputs are stable; negative indices are valid.
- **Consumer audit.** The default grid is uniform open, and nothing live samples its cell data (`world_to_cell`, `sample_cell_data_at_world`, and `EnvironmentFootprintSampler` have test-only callers), so the origin shift's only live effect is this write gate.
...
| **Representation:** unchanged egocentric model — **8-way sector recomputed each tick** from **`last_world_pos − creature_pos`**: **N, NE, E, SE, S, SW, W, NW** (**45°** sectors; **`+Y = N`** matching `ai_driver` notes). Weak **8-way step** bias — **not** a bogus map-fixed compass. (LLM movement control is retired; this sector data has no LLM consumer.)
...
| **`passibility_fail_count`** | int | Failed approach / shelter probe / passibility contradiction | §9 switch bias ([CREATURE_MOVEMENT_V3.md §3](CREATURE_MOVEMENT_V3.md) **C**); clear on re-awareness (**§5.4**) |
...
#### Consult (planner adapter — [CREATURE_MOVEMENT_V3.md §8.4](CREATURE_MOVEMENT_V3.md))
...
**Deferred:** Trait modulation on confidence and read weight ([CREATURE_MOVEMENT_V3.md §6.2](CREATURE_MOVEMENT_V3.md)). Variable bite / pool / sharing — observe `calories_gained` only until plants land; schema allows **`believed_calories_per_action`** vs pool later.
...
**[CREATURE_MOVEMENT_V3](CREATURE_MOVEMENT_V3.md)** supersedes V2 motor merge for ENGINE creatures. Storage contracts remain in this file (**§§5.5, 5.6, 5.7, 10, 14**); V3 owns planner **when** to read/write via the memory adapter (**CREATURE_MOVEMENT_V3 §8.4**).

### 8.4 Planner consumption (V3 adapter — storage vs consult)

**Split:** This file = **row schema, TTL, eviction, salient-write gates**. [CREATURE_MOVEMENT_V3.md §8.4](CREATURE_MOVEMENT_V3.md) = **adapter read/write API** and planner trigger events.
...
| Occluded-in-zone ghost + filtered consults | **`_goal_belief`** read projection — **`consult_danger_samples`**, **`consult_food_targets`**, **`consult_shelter_beliefs`**, **`consult_goal_beliefs`** ([CREATURE_MOVEMENT_V3.md §8.4 consult filters](CREATURE_MOVEMENT_V3.md)) |
...
| `passibility_fail_switch_threshold` | **2** | §9 / V3 switch bias when **`_goal_belief.passibility_fail_count`** reaches threshold ([CREATURE_MOVEMENT_V3.md §3](CREATURE_MOVEMENT_V3.md)) |
...
| `unknown_kind_multiplier` | **1.0** | V3 — no familiarity term; tune when traits / plants land ([CREATURE_MOVEMENT_V3.md §6.2](CREATURE_MOVEMENT_V3.md)) |
...
| `locale_prior_write_blend` | **0.35** | Per-write lerp for **`stored_strength`** toward **`success_rate`** (**§14.2**). **V3: fallback-only** (not in `default_creature_motor_v3_params()`; the store's 0.35 literal applies; V2 ship profile sets 0.32). |
| `salient_write_max_per_sec` | **100** | Per-creature safety valve on salient writes/sec (**§14**). **V3: fallback-only** (store literal 100). |
...
| Planned key | Default (phase-1) | Role |
|-------------|-------------------|------|
| `locale_prior_max_buckets` | **100** | LRU cap on **`LocalePriorMap`** rows per creature. **V3: fallback-only** (store literal 100). |
| `locale_prior_idle_evict_base_sec` | **10** | Base idle time before bucket eligible for eviction. **In V3 defaults since 2026-09-24** (decision 45 D; previously reached only via fallback literal; V2 spine copy kept). |
| `locale_prior_idle_evict_per_attempt_sec` | **1** | Added idle sec per **`attempt_count`** beyond first: **`idle_evict_sec = base + (attempt_count - 1) * per_attempt`**. Single formula: `GoalSourceMemoryStore.idle_limit_for_row`, shared by `_evict_if_needed` and the `avoid_hostiles_cells` read filter (2026-09-24). **In V3 defaults since 2026-09-24.** |
| `locale_prior_idle_evict_avoid_hostiles_sec` | **300** | **Shipped 2026-09-24; in V3 defaults since the decision 45 D fix pass (same day).** Replaces `base` for **`avoid_hostiles`** rows only (rare-escape cadence: a 10 s base would evict a flee row before it could ever be reused); per-attempt term shared. **Wall-clock** seconds (`Time.get_ticks_msec()/1000`, same clock as `last_used_time`), **not** game-time ticks. **Requirement:** if a pause / game-speed control is ever added, this and every other locale-prior clock (all goal kinds) must convert to game time — see [ENHANCEMENT_BACKLOG_PLAN.md](../ENHANCEMENT_BACKLOG_PLAN.md) **Game pause / time-scale** row (Other). Eviction runs only inside `try_salient_write`. **Known trade-off:** the shared **`locale_prior_max_buckets`** cap still evicts by `idle_age / attempts` across **all** goal kinds, so long-lived, few-attempt flee rows are now the first cap-eviction casualties. |
| `flee_locale_bias_bonus` (`creature_motor_v3`) | **0.05** | **Shipped 2026-09-24.** Bonus weight for remembered `avoid_hostiles` cells as flee-pool candidates (**§14.3**); shelter/choke bonuses are **0.15**. Race-gated like them. **Barely matters by design since the decision 45 A fix:** locale cells are scored with true reach, so they cannot beat unobstructed open ground whatever the bonus (§14.3). |
| `flee_separation_gain` (`creature_motor_v3`) | **0.25** | **Shipped 2026-09-24 (decision 45 A).** Every flee candidate scores `+ flee_dist × gain × separation` (separation = clamped change in nearest-threat distance, endpoint vs now). A shelter the creature wins the race to (see `flee_race_won_min_margin`) is credited 1.0. Bounds: ≥ ~0.25 to fix the review repro. Upper bound from the choke fit-gate test: its choke earns ~0.41 raw separation but wins its race (margin ~0.17) and is floored at the 0.5 choke credit, so the test holds up to ~0.326 (~0.279 with the credit at 0). Canonical: [PHYSICS_SQUEEZE.md decision 20](PHYSICS_SQUEEZE.md). |
| `flee_choke_race_won_separation_credit` (`creature_motor_v3`) | **0.5** | **Shipped 2026-09-24 ([PHYSICS_SQUEEZE.md decision 45 J](PHYSICS_SQUEEZE.md)).** A choke candidate the creature wins the race to (same predicate as shelters: margin above `flee_race_won_min_margin`, trait-scaled, since 2026-09-25; was margin > 0) gets `separation = max(separation, clamp(credit, 0, 1))`; lost race / near dead heat unchanged. No race-term floor for chokes. Half the shelter's 1.0: a choke only slows the pursuer, a shelter reached first ends the chase. Max extra = 0.25 × 0.5 = +0.125 × `flee_dist`, below the 0.25 × `flee_dist` a straight-away open bearing earns on this term — more weight, not an automatic win. Pure math: `FleeCandidateScoring.choke_race_won_separation`. |
| `flee_race_won_min_margin` (`creature_motor_v3`) | **0.04** | **Shipped 2026-09-25 ([PHYSICS_SQUEEZE.md decision 45 M](PHYSICS_SQUEEZE.md)).** Minimum worst-case race margin that counts as a **won** flee race (exclusive: `margin > min`), at neutral `change_stability`. One predicate (`FleeCandidateScoring.shelter_race_won`) for all three consumers: shelter separation credit 1.0, shelter race-term floor (decision 45 L), choke credit. Replaces the old `margin > 0` cutoff, which let near dead heats win. Scaled per creature by the two keys below: `min = max(0, base × lerp(scale_min, scale_max, t))`, `t = clamp((cs + 100) / 200, 0, 1)`. Does not affect `race_term` or the belief-bonus gate. Pure math: `FleeCandidateScoring.race_won_min_margin`. |
| `flee_race_won_min_margin_scale_min` / `flee_race_won_min_margin_scale_max` (`creature_motor_v3`) | **0.5** / **1.5** | **Shipped 2026-09-25 (decision 45 M).** Multipliers on `flee_race_won_min_margin` at full Change (−100) / full Stability (+100): 0.02 / 0.04 / 0.06 with defaults. Change tolerates a closer race; Stability sheers off to open ground. **No species `.tres` sets `change_stability` today**, so every live creature uses the neutral 0.04. Trait map: [CREATURE_TRAIT_USAGE.md §3](../Definitive_Features/CREATURE_TRAIT_USAGE.md). |
| `flee_reacquire_window_sec` (`creature_motor_v3`) | **25** | **Shipped 2026-09-24 (decision 45 C).** Re-acquisition failure proxy: a new Flight entry within this many **wall-clock** seconds of a Flight exit, and within one coverage cell of the exit anchor, writes one `avoid_hostiles` `TIER_FAILURE` on the anchor's cell (§14.4). |
| `flee_memory_debug_log` (`creature_motor_v3`) | **true** | **Shipped 2026-09-24 (decision 45 F).** Debug builds only: `FleeMem exit` / `FleeMem reacq` OLog lines (tag `FleeMemory`) and the ` fk= ahc=` explore-log suffix. **Flip to false** in `user://game_config.json` after the decision 44 smoke. |
| `explore_coverage_cell` (`creature_motor_v3`) | **52** (fallback) | Locale cell size for the §2.1.1 compositor on V3 (`coverage_cell_from_motor`, clamped ≥ 16). **V3: fallback-only and never playfield-scaled.** The key is absent from the V3 defaults, so `MotorPlane`'s distance scaler has nothing to scale. Live cells therefore stay 52u on a ~200u field while `dead_end_match_radius` scales to ~5.5. Tracked in the backlog. |
...
| **Preserve / Find blend** (**[CREATURE_MOVEMENT_V2.md §A.3.1](CREATURE_MOVEMENT_V2.md)**): `preserve_bias_food_floor`, `seek_priority_food_ceiling`, `preserve_seek_blend_smoothness` (default **0.5**), **`starvation_override_food_ceiling`** (default **0.10**), **`escape_reversal_window_sec`** (default **1.0** — AH-7 reversal window; **key retained but no V3 consumer since 2026-09-24 — AH-7 suppression dropped, §14.2**). Defaults in **`default_creature_motor_params()`** only.
...
| Jeopardy clears after near-death / damage | Yes (**unshipped — combat-dependent**) | `NEAR_DEATH` → `reward_scalar −1` |
| **Re-acquisition (V3, shipped 2026-09-24):** acute Flight re-enters within `flee_reacquire_window_sec` of an exit, within one coverage cell of the exit anchor | Yes — on the **exit anchor's** cell, at most once per exit | `FAILURE` |
| ~~Starvation override **reversed escape** — creature returns to **any acute danger** within **`escape_reversal_window_sec`**~~ | **Dropped under V3 (2026-09-24)** — see **Reversal detection** below |
| Threat still acute at clear edge | No | `NEUTRAL` |

**Hook (V3, shipped 2026-09-24):** the writer is live. `CreatureMotorStack._on_flight_exited()` (on the `flight_just_exited` tick — the V3 "jeopardy cleared" event, replacing V2 `tactic_jeopardy_egress` true→false) calls **`MemoryAdapter.notify_flight_escape_outcome(body_pos, motor_v3, env_grid)`** → `try_salient_write(GK_AVOID_HOSTILES, &"avoid_hostiles", …)` then `clear_salient_continuation()`. Shape: **anchor = body cell at flight exit** (§2.1.2; write requires the cell inside env-grid bounds — **which today refuses every anchor with x < 0 or z < 0, see the §2.1.1 `<<Comment>>`**); `notify_flight_escape_outcome` returns whether the write landed; **`SUCCESS`** on exit, plus a **`FAILURE`** re-acquisition write (below) since 2026-09-24 (`NEAR_DEATH` stays unshipped, combat-dependent); modality defaults to **`flee_retreat`** because the classifier stub is inactive and the write passes an empty `{}` motor ctx (no `SalientWriteContext` class — see [ENHANCEMENT_BACKLOG_PLAN.md](../ENHANCEMENT_BACKLOG_PLAN.md)). See [PHYSICS_SQUEEZE.md §3 decision 44](PHYSICS_SQUEEZE.md).

**Failure signal — combat-dependent vs not (2026-09-24).**
- **Combat-dependent, still open.** The "near-death / damage → `NEAR_DEATH`" row above is **100% combat-dependent**. Prey defeat is the predator's V3 `EAT` only (`creature_kinematic_body_3d.gd`, "D11 — contact predation retired"), so a caught prey dies with its per-creature memory. There is no herd/shared memory ([PHYSICS_SQUEEZE.md §8g](PHYSICS_SQUEEZE.md)). Only **non-lethal** damage can feed it, which needs combat.
- **Re-acquisition proxy — shipped (user-approved; [PHYSICS_SQUEEZE.md decision 45 C](PHYSICS_SQUEEZE.md)).**
  - **Arming.** `CreatureMotorStack._on_flight_exited` records the exit anchor (body position) and time; the anchor resets in `configure()`.
  - **Firing.** On `flight_just_entered`, `_on_flight_entered` checks that the exit was at most `flee_reacquire_window_sec` ago (25 s, wall clock) and that the body is within one coverage cell of the anchor. If both hold it calls **`MemoryAdapter.notify_flight_reacquired(anchor, motor_v3, env_grid)`**. That runs the same `try_salient_write(GK_AVOID_HOSTILES, …)` as the success write, with `{"tier": TIER_FAILURE}` and an empty `{}` ctx, then `clear_salient_continuation()`. It returns whether the write landed.
  - **At most one failure per exit.** The anchor is cleared when the proxy fires or when the window has expired.
  - **EWMA effect (α 0.15).** A fresh row with SUCCESS then FAILURE → `success_delta` −0.0225 → it leaves the flee pool (§14.3). SUCCESS ×2 then FAILURE → +0.086 → it stays.
  - **Side effect, accepted:** a `safety_met` flicker (exit, then immediate re-entry) counts as re-acquisition.
- **Cornered-during-episode proxy — not implemented.** This option would write `FAILURE` when the flee boxed-in / give-up escalation fires mid-Flight. Only re-acquisition was approved this pass. Tracked in [ENHANCEMENT_BACKLOG_PLAN.md](../ENHANCEMENT_BACKLOG_PLAN.md) (**`avoid_hostiles` locale memory** row, item 1; combat half also on **Combat: `fight` vs `flee_retreat` dominance**).

**Reversal detection (AH-7) — dropped under V3 (2026-09-24).** The V2 design (suppress the write when dominance flipped **Avoid → Find food** mid-episode and the creature re-enters a threat) has nothing to detect: during Flight `MotorGoalHub.build_eligible_goals` returns an empty list when `flight_fast_path_active` and `_run_consideration` clears the incumbent each consideration, so no Avoid → Find-food flip can occur mid-episode, and re-entry of another threat stays inside the same latched episode. The `escape_reversal_window_sec` / `starvation_override_food_ceiling` keys are not consumed by the writer. (Legacy V2 text: [GOAL_DRIVERS §5.1.5](CREATURE_GOAL_DRIVERS.md).)
...
|------|---------------------|
| Rank **all** stored **`avoid_hostiles`** locale rows via **`replay_rank_score`** ([GOAL_DRIVERS §5.1](CREATURE_GOAL_DRIVERS.md)) — **no** `context_hash` filter on rows at consult | Add a **separate flee cardinal cost term** driven only by locale priors (**still true — but see "Reader update" below: cells now enter the flee pool as bonus-only candidates, which is not a cardinal term**) |
...
| Feed **habitual bias** only through normal **`believed_goal_source_bias`** + existing threat motor | ~~Treat locale priors as fake **`Vector2`** flee targets~~ **Reversed 2026-09-24** — see "Reader update" below |

**Implementers:** Threat pass = **ordering + memory consult for replay ranking / modality choice**; cardinal flee still comes from **live threat samples** and **`tactic_jeopardy_egress`** when set. When tactic classifiers are stubbed false, threat pass may no-op beyond ordering until **§A.2.1** flags land. **`consult_threat_response`** does **not** restrict candidates to rows whose **`context_hash`** equals **`consult_context_hash`** in phase-1 (global rank across **`avoid_hostiles`** rows). **Honest status of that consult:** `consult_threat_response` (`goal_source_memory.gd`) still has **zero production callers**, so `preferred_modality` / `replay_rank_score` from the threat pass currently have **no consumer**; the `preferred_modality` consult is unchanged by the reader below.

**Reader update (2026-09-24, Option A chosen by user) — deliberate reversal of the two "Does not" rows above.** §14.3 predates [PHYSICS_SQUEEZE.md](PHYSICS_SQUEEZE.md) decisions 20/39, which widened the flee pool to belief candidates scored by race margin. Remembered `avoid_hostiles` cells now join that pool as a **third** belief-candidate kind after shelter and choke:

- **Read path:** `MemoryAdapter.consult_flee_locale_candidates(creature_pos, motor_v3, now_sec)` → `[{pos, weight}]` from the pure-read `GoalSourceMemoryStore.avoid_hostiles_cells(motor_p, now)`. One entry per cell; weight = best `stored_strength` clamped 0..1. Skipped rows: `stored_strength <= 0` or `success_delta <= 0` (e.g. after a re-acquisition `FAILURE`, §14.4), and, since 2026-09-24, rows idle past `GoalSourceMemoryStore.idle_limit_for_row`. That formula is shared with `_evict_if_needed`, which only runs inside a salient write. The read does **not** bump `last_used_time`. `pos` = cell centre `(cell + 0.5) × coverage_cell_from_motor` on x/z, y from the creature. `now_sec` is wall-clock seconds; negative (default) = read the clock.
- **Planner:** `MotorPlanner._flee_belief_candidates` adds them (shared helper `_flee_score_belief_entries`), scored by the same `FleeCandidateScoring` function with new key `flee_locale_bias_bonus` = **0.05** (shelter/choke 0.15), gated by `belief_race_factor`. They only fill `flee_belief_max_candidates` slots shelter+choke leave unused (nearest first) and go through the same `_flee_candidate_probe` / route-plausibility / decision-43 detour logic.
- **Over-attraction fix (2026-09-24, [PHYSICS_SQUEEZE.md decision 45 A](PHYSICS_SQUEEZE.md)).**
  - **Scoring.** Locale cells are scored with their **true** reach (no rescale to the full flee distance), plus the separation term every candidate now gets (`flee_separation_gain` 0.25).
  - **Filters.** Before capping, cells whose centre is within one coverage cell of the creature (including its own cell) are skipped, as are cells not in the half-plane away from the nearest threat.
  - **Consequence:** a locale cell can no longer beat an unobstructed open bearing. It only matters when geometry cuts open bearings short, so `flee_locale_bias_bonus` barely matters.
  - **Scenario matrix (2026-09-24, [PHYSICS_SQUEEZE.md decision 45 K](PHYSICS_SQUEEZE.md)):** through the real `_mint_flee_waypoint` at defaults, no locale cell beat open ground in any case. Inside a 45u pocket no locale candidate can exist at all: the one-coverage-cell (52u) proximity filter removes every cell.
- **Not a cardinal cost term.** It is a bonus-only candidate in the decision-20 pool; the "no separate flee cardinal term" intent stands.
- **Dead-end exclusion:** a locale candidate is dropped at build time when `_flee_endpoint_is_dead_end` fires for it, **unless** `MemoryAdapter.has_usable_shelter_near` finds any non-`failed` shelter belief within `dead_end_match_radius` (shelter-only rescue; excluded candidates take no cap slot). Since 2026-09-24 choke candidates get the same filter and rescue. Shelter candidates stay exempt (decision 11). This shelter-only rescue for flee destinations is the whole design; [PHYSICS_SQUEEZE.md decision 21](PHYSICS_SQUEEZE.md) was amended to match and its general override was rejected. **Deferred:** `has_usable_shelter_near` also counts `observed` shelters, while the flee pool admits only `confirmed`+ (backlog).
- ~~<<Comment (2026-09-24): reader-effect caveat — belief-candidate reach normalisation lets an ahead-side cell beat open bearings even with zero bonus… Also open: success-only rows cannot unlearn a bad cell until a failure tier exists.>>~~ **Resolved 2026-09-24:** reach normalisation no longer applies to locale cells (decision 45 A), and the re-acquisition `FAILURE` write (§14.4) lets a bad cell unlearn. Live smoke passed 2026-09-25, user-reported ([PHYSICS_SQUEEZE.md Decision 44 live smoke test](PHYSICS_SQUEEZE.md#decision-44-live-smoke-test)). **Open:** whether locale memory should ever beat open ground (backlog).
...
| **Write gates** | Noise control | **Resolved:** outcome-driven **`GoalKind`**; mid-band eat → `find_food`; one write per outcome. (AH-7 reversal suppress **dropped under V3, 2026-09-24** — §14.2) |
...
| **Threat-response algorithm** | Priors vs flee cardinal | **Resolved — §14.3:** ordering only; **`replay_rank_score` / modality choice**; no separate flee cardinal term phase 1. **Amended 2026-09-24:** cells also join the flee pool as bonus-only candidates (§14.3 "Reader update") |
| **Prior decay / forget** | When locale prior buckets fade / drop | **Resolved (phase 1):** soft EWMA in-row; idle evict **`10 + (attempt_count-1)s`** (`avoid_hostiles`: **`300 + (attempt_count-1)s`**, 2026-09-24; one formula, `idle_limit_for_row`); LRU **`locale_prior_max_buckets`** default **100**; **not** belief TTL parity |
...
| Date | Change |
|------|--------|
| 2026-09-25 | **Flee race-won threshold ([PHYSICS_SQUEEZE.md decision 45 L–M](PHYSICS_SQUEEZE.md)).** §10: new rows `flee_race_won_min_margin` 0.04 and `flee_race_won_min_margin_scale_min` / `_scale_max` 0.5 / 1.5 (`change_stability`-scaled; replaces `margin > 0` as the "race won" test for the shelter credit, the new shelter race-term floor and the choke credit). `flee_choke_race_won_separation_credit` row: predicate wording updated, no race-term floor for chokes. `flee_separation_gain` row: upper-bound note corrected (fit-gate choke wins its race and is floored at the 0.5 credit; the test holds to ~0.326, ~0.279 with credit 0). Locale-cell scoring (§14.3) unaffected. |
| 2026-09-24 | **Locale quadrant bug fixed; choke race-won credit; flee matrix note ([PHYSICS_SQUEEZE.md decision 45 I–K](PHYSICS_SQUEEZE.md)).** §2.1.1: **Origin** row split into locale identity (still world-origin 52u floor, negative indices valid) vs the bounds gate; **Out of bounds** row now describes the env grid's own `world_to_cell` test (respects `origin_world`, env cell size, `cell_size <= 0` rejects) and the live default grid's new `origin_world` = playfield min; the quadrant `<<Comment>>` is resolved (root cause: default grid at origin (0,0)). §14.3: "Precondition" note removed; scenario-matrix note added (no locale cell beats open ground; none can exist inside a 45u pocket). §10: new row `flee_choke_race_won_separation_credit` 0.5. |
| 2026-09-24 | **Decision 45 fix pass sync ([PHYSICS_SQUEEZE.md decision 45](PHYSICS_SQUEEZE.md)).** §14.4: re-acquisition `FAILURE` proxy shipped (`notify_flight_reacquired`, `flee_reacquire_window_sec` 25 s wall-clock, one failure per exit; EWMA effect recorded), replacing the "success tier only / no failure signal" wording; `NEAR_DEATH` stays combat-dependent; the cornered proxy is not implemented. §14.3: reader signature `consult_flee_locale_candidates(creature_pos, motor_v3, now_sec)` / `avoid_hostiles_cells(motor_p, now)` with the idle-limit age filter (`idle_limit_for_row`); locale cells use true reach + own-cell/half-plane filters (can no longer beat open ground); choke candidates get the same dead-end filter; the reader-effect `<<Comment>>` is resolved. §10: `locale_prior_idle_evict_*` now in the V3 defaults; new rows `flee_separation_gain` 0.25, `flee_reacquire_window_sec` 25, `flee_memory_debug_log` true, `explore_coverage_cell` (V3 fallback-only, never scaled); `locale_prior_max_buckets` / `locale_prior_write_blend` / `salient_write_max_per_sec` marked V3 fallback-only. §2.1.1: new `<<Comment>>` for a pre-existing, unfixed bug — `anchor_cell_in_bounds` refuses every locale write outside the +x/+z quadrant of the origin-centred playfield. |
| 2026-09-24 | **Doc-only follow-ups to the avoid_hostiles writer.** Under the `avoid_hostiles` outcomes (heading **14.4**, after the writer hook) added a note: the `NEAR_DEATH` / damage failure signal is 100% combat-dependent (prey defeat = V3 `EAT` only, no shared memory), and two non-combat `FAILURE` proxies (re-acquisition, cornered-during-episode) are open pending a user decision. §10 `locale_prior_idle_evict_avoid_hostiles_sec` row now records a game-time requirement if pause / game-speed is ever added, pointing to the new backlog **Game pause / time-scale** row. No code change. |
| 2026-09-24 | **`avoid_hostiles` locale writer + flee reader shipped:** §14.2 hook = `flight_just_exited` via `notify_flight_escape_outcome` (success tier, body-cell anchor, `{}` ctx); AH-7 reversal suppression dropped (goal table empty during Flight); §14.3 reader Option A (locale cells as third flee-pool candidate kind, reversal of the "no fake flee targets" row; `consult_threat_response` still no production callers); §10 keys `locale_prior_idle_evict_avoid_hostiles_sec` (300, wall-clock) and `flee_locale_bias_bonus` (0.05). See [PHYSICS_SQUEEZE.md decision 44](PHYSICS_SQUEEZE.md). |
