...
| [C8](#c8-stable-pre-existing-test-failures-found-during-r1-mitigation-2-audit) | Stable pre-existing test failures found during R1 mitigation #2 audit | `in_progress` — 11/13 fixed, 2 open | unassigned |
...
**Status:** `in_progress` — 11 of 13 fixed, 2 open  
...
- **Not part of this trio, still open:** the `two known bushes` (ObjectDB-adjacent) and `belief north of origin` (wedge-scoring) failures below — never shared a root cause with the LOS trio, untouched this session.

Verified: full headless suite **6 → 2** failures, byte-identical across 2 consecutive `--headless` runs (invariant harness temporarily disabled per the existing `run_all.gd`/synthetic-fixture workaround, reverted after).
