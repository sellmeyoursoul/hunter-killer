...
- [ ] **Decision needed:** attempt further escalation (e.g. a give-up-and-try-something-structurally-different fallback like explore's `boundary_scan`, for when even the best-reaching geometry-scored candidate is still bad), dig into why run-to-run variance exists despite a deterministic forced spawn, or accept current state as a large, verified improvement and move on / revisit alongside the R1 architecture-risk work.
