...
| [C8](#c8-stable-pre-existing-test-failures-found-during-r1-mitigation-2-audit) | Stable pre-existing test failures found during R1 mitigation #2 audit | `in_progress` — 5/13 fixed | unassigned |
| [C9](#c9-flee-waypoint-latch-corrupted-by-reactive-backtrack-deflection-rabbit-stuck-at-playfield-edge) | Flee-waypoint latch corrupted by reactive backtrack deflection (rabbit stuck at playfield edge) | `done` | unassigned |
...
**Status:** `in_progress` — 5 of 13 fixed 2026-07-16, 8 open  
...
- **`_test_motor_planner_explore_post_scan_inward_align_no_flip_flop`** ("post-scan inward align converges to MOVE within 16 ticks") — **likely a real bug, highest priority of this group.** Debug trace: body starts facing away from the inward waypoint, correctly turns 8× 22.5° increments to reach *exact* alignment (`dot≈1.0`) at tick 8 — then keeps turning the same direction for 8 more ticks, spinning a full circle, never once selecting `MOVE_FORWARD` even at perfect alignment. `boundary_scan_active` flips from `false` (ticks 0-2, matching the test's explicit `_end_boundary_scan` setup) back to `true` at tick 3 onward, routing every subsequent tick through `_boundary_scan_action` instead of the normal align/move path. Something re-arms boundary-scan mode after only 2-3 turn-only ticks, before the body ever gets a chance to move forward — this is exactly the "Fix 2 (Fox rim)" flip-flop scenario this test's own docstring describes, so it reads as a regression in the boundary-scan re-entry guard, not a stale assumption. Next step: trace what sets `boundary_scan_active=true` again inside the tick loop (likely a rim-proximity re-check in `select_action` or `_sync_step_objective`) and whether it should be respecting `boundary_scan_egress_ticks` or similar egress-grace state instead of re-arming immediately.
...
- [ ] Root-cause and fix `post-scan inward align` (boundary-scan re-arm) — likely real regression.
...
**Status:** `done` — fixed and verified 2026-07-16  
**Slice:** unassigned — [`motor_planner.gd`](../../creature/motor/motor_planner.gd), `apply_immediate_blocked_path_reevaluation`  
...
- [ ] Duel manual re-verification (rabbit no longer locks into a limit cycle at the playfield edge) — not yet re-run live; next duel smoke should confirm.
- [ ] Fox's own wall-oscillation (same `_run_path_clearance_los_nav` family) — tracked as a C8 follow-up, not fixed here.
