...
| [C10](#c10-fox-ends-up-under-the-geography-after-close-contact-with-prey-new-2026-08-05) | Fox ends up under the geography after close contact with prey | `open` — found, not investigated | unassigned |
...
- Are there other flaky assertions in the suite not yet caught because no one has run it 6× back-to-back? Worth a scripted multi-run check before trusting any future single-run headless diff as authoritative.

---

## C8 — Stable pre-existing test failures found during R1 mitigation #2 audit

**Status:** `in_progress` — 7 of 13 fixed, 6 open  
...
### Open (not fixed — needs dedicated follow-up)

- **`_test_locomotion_executor_move_blocked`** ("MOVE_FORWARD against wall sets blocked") — debug trace shows the body's X position already 2+ world units past a 0.5-unit-thick wall by the very first logged tick, with `on_wall=false` and `on_floor=false` for all 30 ticks (continuous freefall, never resting on the test floor either). Runtime `collision_layer`/`collision_mask` were verified correct (body mask=1 matches floor/wall layer=1 — the `.tscn` file's layer=2/mask=4 defaults are overridden by `_apply_physics_layers()` in `_ready()`, a dead end investigated and ruled out). Root cause not found — reads as a Jolt physics substep/tunneling issue possibly specific to this machine/Godot version (this session involved two machine moves, see recent commit messages), consistent with C7's untriaged Jolt-nondeterminism hypothesis, but this failure is *stable* not flaky, so may be a distinct issue. Needs a dedicated physics-repro session, ideally on a different machine to rule out environment drift.
- **`_test_motor_explore_seek_wall_bias_opens_away`** ("wall north of spawn-facing picks open bearing away from obstruction") — `_nav_clearance`'s LOS raycast (`_AwarenessZone.line_of_sight_clear`) reports **zero occlusion in both directions** (`nav_clear_south=1.0, nav_clear_north=1.0`) despite a wall placed 4 units away directly in the raycast's path — the wall is never detected at all. Same collision-detection-not-registering family as the wall-block test above; likely the same root cause once found.
- **`_test_memory_adapter_ghost_danger_without_live_los`** + **`_test_creature_motor_stack_safety_blocked_by_ghost`** ("occluded threat ghost emits danger sample" / "ghost danger blocks Safety state") — same LOS-occlusion-raycast family as the wall-bias test above (both use a freshly-spawned `StaticBody3D` wall + `await physics_frame`, and both depend on `line_of_sight_clear`/raycast collision detection). Likely the same root cause.
- **Root-caused, not part of this trio (2026-08-05):** [C9](#c9-flee-waypoint-latch-corrupted-by-reactive-backtrack-deflection-rabbit-stuck-at-playfield-edge)'s duel re-verification caught fox + rabbit both stuck at the playfield boundary, each ping-ponging between exactly 2 waypoints (`blk=1`, `cblk` cycling 0→1→2→0, never escalating). Originally guessed to be the same `_run_path_clearance_los_nav`/navmesh-first-waypoint instability as this trio — **disproven** by forcing an on-demand repro (temporary edge-pinned duel spawn) and instrumenting that exact function: it never fired once in the whole session (`NAV_DBG` count = 0), so the LOS raycast/nav resolution path was never even invoked. The real cause is two separate, unrelated mechanisms — `_maintain_flee_latch` discarding the rabbit's one-tick escape deflection every following tick, and `_remint_alternate_pursuit_detour`'s ±60° alt-flip finding both sides blocked at a corner for the fox — see C9's "Root-cause correction (2026-08-05)" section for the full trace. **Not** the same family as this trio's raycast-non-detection issue; removed from this list as a fix candidate for whoever picks up this trio next.
- **`_test_creature_motor_stack_food_map_confidence_inventory_ratio`** ("two known bushes yield partial inventory_ratio") — first assertion in the same test (3 known bushes → confidence 1.0) passes; after erasing one belief and re-ticking, confidence should drop to 2/3 but doesn't. `count_known_objectives` → `_belief_instance_passes_diet` uses `instance_from_id`-style object lookups that appeared in this session's error output as `Condition "slot >= slot_max" is true` (ObjectDB exhaustion) — plausibly a downstream effect of C4's already-tracked residual ObjectDB noise from running ~6000 lines of tests in one process, not a food-map-confidence logic bug. Not confirmed.
- **`_test_memory_adapter_count_known_objectives_fractional`** ("belief north of origin scores higher in north wedge than east wedge") — pure logic test (no physics/bodies involved), not yet investigated beyond confirming `_bearing_wedge_index`'s angle convention correctly maps north to wedge 0 and east to wedge 2 (matching the test's own labels). The scoring gap itself (why wedge 0 doesn't outscore wedge 2 despite a `seed_locale_prior_for_test` boost specifically on wedge 0) needs a follow-up look at `explore_bearing_coverage`'s aggregation.
...
- [ ] Root-cause the LOS/wall raycast non-detection shared by 3 tests (wall-block, wall-bias, ghost-danger ×2).
- [ ] Root-cause the `two known bushes` ObjectDB-adjacent failure (may fold into C4).
- [ ] Investigate `belief north of origin` wedge-scoring gap.

---

## C9 — Flee-waypoint latch corrupted by reactive backtrack deflection (rabbit stuck at playfield edge)

**Status:** `in_progress` — original bug fixed and verified 2026-07-16; **reopened 2026-08-05** after live re-verification found the fix's own documented open gap (fox's wall-oscillation) live, then found the initial follow-up hypothesis wrong and root-caused + fixed two real mechanisms (3 iterations); live loop-closure still unconfirmed — see "Root-cause correction" and "Fix shipped" below  
...
- [~] Live re-verification (3rd fix): **inconclusive** — first-ever full contact between the pair occurred (encouraging), but session was interrupted by the unrelated C10 bug before the flee-loop question could be answered cleanly. Needs another live pass.
...
**Status:** `open` — found, not investigated  
**Slice:** unassigned — likely physics/collision (Jolt `CharacterBody3D`), not planner logic; possibly related to [C3](#c3-prey-contact-without-eat--body-pin-stall-fox)'s EAT-contact geometry or the [C8](#c8-stable-pre-existing-test-failures-found-during-r1-mitigation-2-audit) LOS/wall-raycast-and-physics family, not confirmed  
...
- [ ] Root-cause the fall — confirm whether it's a `move_and_slide` overlap-resolution tunneling issue, an EAT-range/C3 interaction, or something else.
- [ ] Reproduce reliably (or determine it's rare/one-off) before attempting a fix.
