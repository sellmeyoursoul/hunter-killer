...
|----|-------|--------|-------|
| [C1](#c1-pursuit-contact-geometry-stall-fox) | Pursuit contact geometry stall (fox) | `done` — headless green (incl. 2026-09-24 straight-line-to-prey regression coverage); duel-validated via accumulated live pursuit evidence, most recently 2026-09-24 corner-scenario smoke captures | `post-6d-approach-geometry` (shared) |
...
| [C24](#c24-gk_avoid_hostiles-ignored-its-own-explore-fallback-seed-and-sawtoothed-forever-east-wall-spinning) | `GK_AVOID_HOSTILES` ignored its own explore-fallback seed and sawtoothed forever (east-wall "spinning") | `done` | unassigned |
...
**Status:** `done` — reopened 2026-09-24 on a live "both stuck" repro (wolf/rabbit); straight-line-to-prey candidate shipped for the live-pursuit detour escalation (see "Reopened 2026-09-24" below). Dedicated headless regression coverage now exists (`_test_motor_planner_pursuit_detour_prefers_straight_line_when_it_reaches_farther`, see "Reopened 2026-09-24" below), and the maintainer reported multiple successful live duel smoke-test captures in corner scenarios after the fix shipped. Previously marked `done` 2026-08-06 on accumulated live-duel evidence rather than a dedicated rock-pinch repro (see "Duel validation" below).  
...
- [x] Headless: straight-line-to-prey candidate in `_remint_alternate_pursuit_detour` actually wins over the blind ±60° rotation when it reaches farther on a real navmesh (`_test_motor_planner_pursuit_detour_prefers_straight_line_when_it_reaches_farther`) — added 2026-09-24, see "Reopened 2026-09-24" below.
...
### Reopened 2026-09-24 — "both stuck" repro (wolf/rabbit), decision-42 interaction, straight-line-to-prey mint

**Repro evidence:** `.smoke_logs/smoke_both_stuck.log` (wolf#211 vs. rabbit, `src=live gk=find_food`, prey instance `211543920502`). Over `t=994`–`1071`+ the wolf's `ult`/`udist` (the rabbit's live position / distance to it) barely move (`udist` ≈ 21.3–22.5, drifting up not down — zero net closing progress), while `tgt`/`dist` (the minted detour waypoint) alternates between exactly two fixed points ~20–22 units out, in lockstep with `cblk` cycling `0 → 1 → 2 → 0`. This is `_remint_alternate_pursuit_detour`'s escalate-then-give-up shape (`pursuit_detour_max_escalations` = 2) repeating forever, matching this doc's original C1 pinch-stall shape exactly — not a new failure mode.

**New finding — decision 42 defeats the give-up backstop for moving prey:** `GoalBeliefMemory._upsert_row` (`goal_belief_memory.gd:182`, decision 42, 2026-09-22) resets `passibility_fail_count`/`last_passibility_fail_ms` to 0 on every observation *only* for non-moving targets — a moving target's count still resets every tick it stays visible, by design ("moving prey pursuit has its own separate detour/give-up path anyway"). That separate path is exactly `_remint_alternate_pursuit_detour`'s escalate-then-give-up — but its "give-up" (`pursuit_detour_gave_up`) only clears the latch and lets **one** `apply_blocked_objective_resolution` tick through; it does not, on its own, ever accumulate toward `_food_pursuit_exclusions`' `passibility_fail_switch_threshold` for a continuously-visible moving target, because decision 42 keeps wiping that same counter every intervening observation tick. Net effect confirmed in the log: the wolf never excludes this rabbit and never switches food targets — it re-mints the identical ±60° pair forever with no escape valve.

**Q1 — is `blk=1` real physics contact or look-ahead prediction?** Real, post-hoc, per-tick physics contact — not a forward-looking shapecast/raycast. Traced: `ActionOutcome.blocked` (`action_outcome.gd`) is set from `LocomotionExecutor._displace_along_facing` (`locomotion_executor.gd:159`), which calls `body.move_and_slide()` (or `apply_horizontal_move_intent`) *then* checks `_is_move_blocked` (actual displacement shortfall / `is_on_wall()`) and, separately, `was_ghost_layer_blocked_last_move()` (a real per-tick ghost-layer contact stop, decision 25 — also physical, just on the query-only layer, not a prediction). `motor_planner.gd`'s `consecutive_blocked` counter (feeding `cblk`) increments only from that same `executor_blocked`/`move_stuck` result. `MotorPathClear.resolve_step_objective` and the `_flee_candidate_probe`/`_route_scanned_endpoint`/`RoutePlausibilityScan` family (`motor_path_clear.gd`, `motor_planner.gd`) are look-ahead — `NavigationServer3D.map_get_path` plus a route shapecast — but they only ever choose *candidate waypoints*; they never set `blk`/`ActionOutcome.blocked` or drive `cblk`. So in this repro, `blk=1` means the wolf actually made real contact each of those ticks, not that a candidate was pre-rejected for a wall still several units away.

**Q2 — mint straight at the prey vs. predict wall-slide and lead it:** Investigated both; implemented (a), the conservative one, per this doc's own guidance to prefer the narrower fix given C1's history of subtle regressions in this exact area.

- **(a) shipped:** `_remint_alternate_pursuit_detour` (`motor_planner.gd`) previously only ever rotated the *pursuer's own* blocked bearing ±60° — it never tried a fresh straight line at the prey's actual live `step_ultimate_pos`, even though `_apply_live_food_objective` aims exactly there on every non-detoured tick. Added a reach-scored comparison using the same `_flee_candidate_probe` + `_apply_route_plausibility_scan` primitive the flee side already uses to rank its own bearing candidates (per this session's question about reusable PHYSICS_SQUEEZE machinery — the primitive itself turned out reusable as-is; no new choke-point/squeeze-belief plumbing needed): probe a straight line to `ultimate` and the existing ±60° rotated candidate, and take whichever's navmesh+route-scan `reach` is strictly greater. Ties (including every synthetic fixture with no baked navmesh, where `_flee_candidate_probe` returns the full requested distance for both) keep today's rotated pick unchanged — the change only fires when a real navmesh shows the straight line clearly reaches farther.
- **(b) not attempted:** a lead/intercept mint predicting wall-slide from the prey's velocity was judged more speculative and not needed to close (a) — deferred unless (a) proves insufficient in a fresh duel/repro pass.
- **Not reused as-is:** the flee-side choke-point/squeeze-gap belief machinery (`creature/motor/choke_point_*.gd`, decisions 19–23) is a *shelter-seeking* primitive (scores candidate flee bearings/shelter beliefs for the fleeing creature's own benefit) — nothing pursuit-side currently detects "target is geometrically pinned" as a positive signal to exploit; the reach-scored straight-line candidate above gets the same practical effect (an unrotated shot at the prey succeeds when the prey is genuinely cornered) without needing a new belief type. A dedicated "prey is pinned" detector remains a possible follow-up if (a) alone doesn't fully close future repros.

**Verification:** Isolated the diff via `Edit`-tool revert/restore (not `git stash`, which would have also reverted this session's other uncommitted work) and ran the full headless suite (`godot --path . --headless -s res://tests/run_all.gd`) both ways — pre-change and post-change both produced the identical single pre-existing failure (`ASSERT: stack blocked move writes passibility, dead-end, or §9 action`, `_test_creature_motor_stack_blocked_memory_writes`), confirmed unrelated (present with the fix fully reverted). `python tools/check_gdscript_no_tabs.py` clean.

**Regression coverage added (2026-09-24, `test-harness`):** `_test_motor_planner_pursuit_detour_prefers_straight_line_when_it_reaches_farther` (`tests/run_all.gd`) — a surgical unit-style test calling `apply_immediate_blocked_path_reevaluation` (→ `_remint_alternate_pursuit_detour`) directly on a custom baked navmesh split into two disconnected islands by a full-width wall: the pursuer's own +60° rotated bearing lands in the unreachable north island (`reach` clamps to the wall's near edge), while the straight line to the live prey's `ultimate` position stays fully open in the south island. Asserts the newly-minted detour waypoint lands near the prey (south of the wall), not snapped against the blocked rotated bearing. Chosen over extending `_test_repro_rabbit_cornered_north_wall_live_pursuit` because that fixture's real dual-motor-stack, real-navmesh setup doesn't give deterministic control over which of the two candidates (straight vs. rotated) actually wins on a given tick — a direct call against the planner internal, mirroring this file's existing `_test_motor_planner_pursuit_detour_alternate_on_persistent_block` / `_test_motor_planner_pursuit_detour_gives_up_on_dead_end_alternate` pattern, gives a deterministic, lower-flakiness-risk assertion instead. Validity-checked: reconstructed the pre-fix rotated-only candidate shape via a throwaway scratch script calling the same still-unchanged helper functions (`_route_scanned_endpoint`, `MotorPathClear.resolve_step_objective`) against the identical fixture geometry (without editing `motor_planner.gd`, which was off-limits for this task) — confirmed it lands the waypoint on the wall (`z≈1.5`, `dist_to_prey≈13.2`), which fails both of the new test's assertions, then confirmed the actual fixed code passes them. Full suite re-run post-addition: identical single pre-existing failure, zero new failures. `python tools/check_gdscript_no_tabs.py` clean. Duel manual re-verification: maintainer reported multiple successful live smoke-test captures in corner scenarios after this fix shipped.
...
## C20 — Live-vs-locale food handoff had no distance awareness (rabbit stuck right outside a shrub cluster)

**Status:** `done`
**Slice:** unassigned — found 2026-09-16 during multi-predator live testing ([CM_V3_MULTI_MOBS.md](CM_V3_MULTI_MOBS.md)); user report: rabbit looked "stuck right outside the circle of shrubs."
**Evidence:** `motor_explore_tick.log` — rabbit at `t=1147-1154` closing distance on a live shrub (`tgt=(3.0,1.8)`, dist 6.99→6.37, `src=live`), then `t=1155` an abrupt switch to `src=locale` targeting `(26.0,26.0)` (dist jumps to 27.28), a 24-tick detour, then `t=1179` reverts to the *same* live shrub but now at dist=7.36 (worse than before) and facing the wrong direction (`err=-143.6°`).

### Root cause

`_live_vs_locale_handoff_prefers_live` (Pass 4, C2 above) compared live vs. locale candidates by pure calories-per-EAT, with no distance/travel-cost term at all. A live shrub 6.4 units away — about to be reached — lost to a remembered locale target 27 units away because the remembered kind's estimated yield was marginally higher. The creature then reversed ~24 ticks later, net *further* from the shrub than when it started.

### Fix

Redesigned with the user per explicit rules, confirmed in discussion before coding. A locale candidate is, by construction, never something currently observed (a visible match would already be the live candidate) — so the function now **defaults to live** and hands off to locale only for one of two survival/economics exceptions, deliberately not personality-weighted:
- **(A) Net loss** — live's own travel cost (new `_travel_calorie_cost`, the real V3 flat-`move_calorie_per_sec`-while-moving model — the `calorie_cost_per_unit_moved` distance-linear model is dead code once V3 action calories are enabled, so it is deliberately not used here) exceeds its yield.
- **(B) Starvation risk** — live's full round trip (there, eat, back — modeled as 2× one-way cost, the only concretely computable proxy for "resume searching" without knowing where that search goes next) would leave calories at/below a safety margin (`food_handoff_starvation_margin_frac`, default 0.125× `caloric_needs` — a buffer, not exactly 0, since travel cost here is a straight-line/constant-speed estimate) while locale's one-way trip would not.

**Follow-up (2026-09-16, same day):** `change_stability` now scales Exception B's margin 0.5x-1.5x (`food_handoff_starvation_margin_scale_min`/`_max`, new `_food_handoff_starvation_margin` helper, mirroring `_effective_prey_engagement_latch_ticks`'s `t = (change_stability + 100) / 200` convention) — Stability leans on a bigger buffer and peels off to known/locale ground sooner under pressure; Change runs the buffer thinner and pushes further toward live/frontier food before giving up. Deliberately scoped to Exception B only, discussed with the user first: a "tune-only" bias (scale the survival exception) was chosen over a "true preference override" (letting personality alone hand off away from safe, net-positive live food) specifically to avoid reopening this exact bug via a different, trait-gated mechanism — true preference-based handoff, if wanted later, needs its own anti-oscillation design and is left a separate follow-up. Exception A (net loss) left unscaled — a trip that loses calories is a bad trip regardless of personality.

**Second follow-up (2026-09-16, same day): `stat_observation` yield-estimate noise.** User asked whether to also jitter *distance* estimates (flagged as potentially reusable for future ranged combat). Discussed first: every distance used in this handoff is exact straight-line math off a known position (currently observed, or recalled from memory) — never actually a guess — so distance-estimate noise has no natural home in this function; it was scoped out as a separate, general perception-accuracy system for later (ranged-combat targeting, threat-range misjudgment), not built here. The genuine guess in this function is the *yield* (calories-per-EAT), for both live and locale — so noise applies there only. New `_food_yield_estimate_noise_frac(traits, motor_v3)` uses the same `_StatMath.peg_curve(stat_observation, v1, v10, v25)` three-peg tool already used for D10's engagement latch and the turn-rate curve, pegged at stat 1/10/25 (`food_yield_estimate_noise_frac_v1/_v10/_v25`, defaults 0.4/0.15/0.03); `_apply_food_yield_noise` jitters each estimate independently (`value * (1 + randf_range(-frac, frac))`, same `randf_range` chaos convention as `goal_consideration_chaos`) before either exception is evaluated, so a poor observer can occasionally trip (or miss) an exception a perfect estimate wouldn't have. User confirmed, against the D10/latch precedent, that stat 10 (both current archetypes' default) should carry the *same configured moderate noise* other chaos knobs use, not a silent zero — so this is a live behavior change for every creature at default stats, not just Observation outliers. Also threaded `stat_observation` into `creature_motor_stack.gd`'s `_traits_from_body()` (it was building the `traits` ctx dict from only the four personality axes — `explorer_builder`/`change_stability`/`compassion_self_interest`/`community_individual` — `stat_observation` was never included, so without this the new noise function would have silently used its own hardcoded-neutral fallback for every real creature regardless of their actual stat).

### Verification

- 8 handoff-path tests green: 2 pre-existing behavior preserved, 3 for the base fix (close-live-beats-richer-far-locale regression for the exact reported bug, net-loss exception, starvation-risk exception), 1 for the `change_stability` follow-up (neutral baseline, Stability tips into the exception, Change stays with live), 2 for the `stat_observation` follow-up (`_food_yield_estimate_noise_frac` pegs exactly to configured v1/v10/v25 and defaults missing `stat_observation` to 10 — deterministic, no RNG in the curve lookup itself; `_apply_food_yield_noise` is an exact no-op when fracs are zeroed). The 4 direct-call handoff tests that assert exact borderline outcomes now explicitly zero noise fracs (new `_zero_food_yield_noise` test helper) since noise is nonzero at default stat — same convention C7 established for `goal_consideration_chaos`.
- Full headless suite: 3 consecutive runs, only the pre-existing non-deterministic ObjectDB flake (`slot >= slot_max`), no new failures.
- Live smoke-tested 3600 ticks clean (base fix only); `change_stability` and `stat_observation` additions are pure math/config/plumbing on the already-verified handoff path, not independently smoke-tested.

---

## C21 — Live food pursuit permanently stuck against a two-boulder pinch (awareness-cone dropout starved the pursuit-detour latch)

**Status:** `done`
**Slice:** unassigned — found 2026-09-16 during duel playtesting; user report: rabbit "appeared to get stuck between two boulders."
**Evidence:** `motor_explore_tick.log`, `t=915-1114` — the *entire* ~18-second round the user ended manually (`cause=end_ai`). `udist` (distance to the live food target) never left a 20.66-20.76 band the whole round — zero net progress. Pattern repeating every ~7-16 ticks: `src=live` beelines at the food, hits the boulders (`blk=1`), one tick of `src=explore` at a short unrelated fallback point (`cblk` resets to 1, never accumulates), then immediately back to `src=live` aimed at the *exact same raw* food position — walking straight back into the pinch.

### Root cause

The awareness/LOS cone feeding `_AwarenessScan.best_ready_food_target` (`_AwarenessZone.membership_with_los`, gated on `in_awareness`) can drop a live food target for a single consideration tick without the food having moved or been consumed — most visibly while a blocked live pursuit is mid-turn against obstructing geometry. `_derive_find_food_step_objective` treated that one-tick dropout as "food gone" and handed off to generic `_mint_explore_objective_for_goal`, which stamps `step_source = "explore"`. `_maybe_mint_pursuit_detour_latch` (the C1 mechanism that's supposed to remember a block and route around it) only arms while `step_source == "live"`, so the block that caused the dropout was never recorded. The very next tick, the food reappears in the scan and the unconditional live-food maintenance branch (`elif not live_food.is_empty() and step_source != "locale"`) reapplies the same raw, still-blocked position with no memory of what just happened. Repeat forever — a coupling gap between the transient-dropout/explore-fallback mechanism and the C1 pursuit-detour latch, neither of which knew about the other.

### Fix

`_derive_find_food_step_objective` now coasts through a short grace window instead of discarding an active live pursuit on a single empty scan: new `live_food_awareness_grace_ticks_remaining` state field, refilled to `live_food_awareness_grace_ticks` (default 2) every tick a live target is actually found, decremented (not refilled) on each consecutive empty-scan tick while `step_source == "live"` and a live objective is already held. While budget remains, the function returns without touching `step_goal`/`step_source` — a true no-op hold, so `step_source` stays `"live"` and the very next blocked tick still lets `_maybe_mint_pursuit_detour_latch` arm normally. Once the budget is exhausted (food genuinely gone — eaten, despawned, out of range), it falls through to the existing explore/locale-search fallback unchanged; the grace window is bounded, not a permanent hold.

### Verification

- New test `_test_motor_planner_live_food_awareness_grace_holds_through_transient_dropout`: direct-calls `_derive_find_food_step_objective` with a hand-seeded "already live-pursuing" state and an empty scan for `live_food_awareness_grace_ticks` consecutive ticks — asserts `step_source` stays `"live"` and `step_goal` is untouched throughout, then asserts a further empty tick (budget exhausted) falls through to a non-`"live"` source, proving the hold is bounded.
- Full headless suite: 3 consecutive runs (baseline-stashed comparison included) — only the same pre-existing `slot >= slot_max` ObjectDB flake, byte-identical to the unmodified baseline; no new failures.

---

## C22 — Resting creature never forms a shelter belief (`REST` missing from the opportunistic-observation gate)

**Status:** `done`
**Slice:** unassigned — found 2026-09-16 during duel playtesting; user report: a rabbit resting at the edge of a shrub ring fled into the open instead of toward the ring when two foxes approached, asking whether shelter had ever been "minted" and if so why flight won anyway.
**Evidence:** `motor_explore_tick.log` — `gk=rest` for several ticks at the shrub-ring position, then an immediate jump straight to `gk=avoid_host` the moment a fox entered awareness; `gk=shelter` never appears anywhere in the trace, before or after.

### Root cause

`_maybe_observe_shelter_opportunistically()` ([creature_motor_stack.gd:1203-1223](../../creature/motor/creature_motor_stack.gd#L1203-L1223)) is the passive/no-commitment mechanism (2026-09-11 design review) meant to let a shelter belief exist for a spot "the creature only ever visited while too hungry for `GOAL_SHELTER` to have won arbitration" — exactly this scenario. But its call-site gate only fired on `MotorAction.EAT`, `STAY`, or `WAIT`. `MotorAction.REST` was split out of generic `STAY` on 2026-08-26 (this session's earlier C19 fix) — about two weeks *before* the opportunistic-observation gate was written, but the gate was never updated to include it. A resting creature — the single most obvious case of "lingering somewhere for its own reasons" — silently never triggered the check, so `shelter_map_confidence` stayed at 0 for that location no matter how long it rested there. Separately (not fixed here, tracked as a follow-up): even a confirmed shelter belief's weight is flat (`goal_base_shelter * max(confidence, goal_shelter_explore_floor)`) and doesn't scale with threat proximity the way `GOAL_AVOID_HOSTILES`'s `urgency_flight` does, so flight would likely still outrun shelter once real threats are closing regardless of confidence — a design gap, not a bug, deferred until this fix is verified.

### Fix

Added `MotorAction.REST` to the action check at `creature_motor_stack.gd:242-253`. Considered making the check unconditional (any action) instead, but rejected: the probe (`_ShelterProbe.enclosure_fraction`) costs 8 raycasts per call, and `EAT`/`STAY`/`WAIT`/`REST` are exactly the actions where the creature holds its position — that keeps the check occasional rather than running on nearly every tick during ordinary locomotion (`MOVE_FORWARD`/`TURN_*`), where the position is transient and a probe there is both costlier and less meaningful.
...
1. **Dead-end marks** ([CREATURE_MEMORY.md §5.6](CREATURE_MEMORY.md), `dead_end_memory.gd`) — position + approach-heading keyed, 15s TTL, capped at 12, ~~belief-overlap override~~ (**resolved 2026-09-24:** no general override exists or is intended. [PHYSICS_SQUEEZE.md](PHYSICS_SQUEEZE.md) decision 21 was amended, user-approved, to the shipped design: the plain `is_waypoint_dead_end` query, plus a shelter-only rescue for locale-cell and choke flee destinations only). The only mechanism with a real forget-and-retry story.
...
**Resolved 2026-09-24 (user-approved decision 21 amendment in [PHYSICS_SQUEEZE.md](PHYSICS_SQUEEZE.md)):** "wired the shared dead-end check" below means the plain `is_waypoint_dead_end` query (mark within `dead_end_match_radius` plus the heading-dot test). That is now the intended design, not a partial one, so this pass's "closed the wiring gap" conclusion stands unqualified.
- **General override rejected.** Decision 21's original any-kind / any-tier belief-overlap override was superseded, not deferred. An any-kind override would let a food belief rescue the very dead end its own unreachability recorded; an any-tier one would let `failed` beliefs rescue the spots that failed.
- **Only rescue kept.** A shelter-only one for flee destinations: remembered `avoid_hostiles` locale cells, plus choke candidates since the decision 45 B fix pass. Shelter candidates stay exempt (decision 11).
...
## C24 — `GK_AVOID_HOSTILES` ignored its own explore-fallback seed and sawtoothed forever (east-wall "spinning")

**Status:** `done`
**Slice:** unassigned
**Evidence:** live report (2026-09-23, east wall): a fleeing creature "spinning" in place near a wall corner instead of ever actually escaping. Headless repro added below (`_test_motor_planner_avoid_hostiles_blocked_seek_holds_explore_no_sawtooth`), validity-checked to fail without the fix.

### Symptom

A creature under `GK_AVOID_HOSTILES` with a wall-blocked direct-line flee bearing never actually walked an escape route — it visibly oscillated near the wall instead of routing around it, unlike `GK_FIND_FOOD`/`GK_SHELTER`/`GOAL_REST`, which all successfully fall back to the generic explore latch when their own primary objective is unreachable.

### Root cause

`apply_blocked_objective_resolution`'s §9 SEEK fallback (`_seed_explore_after_seek`, `motor_planner.gd`) fires after `dead_end_record_min_blocked_ticks` (default 3) consecutive blocked ticks: it clears `step_goal_set` and latches `step_source = "explore"`, expecting the *next* tick's `_sync_step_objective` to mint an escape waypoint via the shared explore machinery (`_mint_explore_objective_for_goal` / `_maintain_explore_latch`) — the same machinery `GK_FIND_FOOD`, `GK_SHELTER`, `GOAL_REST`, and the default goal-kind case all already had a `step_source == &"explore"` branch to hand off into.

`_sync_step_objective`'s `GK_AVOID_HOSTILES` arm had no such branch. Its only condition — `if refresh_targets or not has_step_goal` — unconditionally re-derived `_flee_objective()` whenever it was true, and the SEEK seed's own `step_goal_set = false` made `not has_step_goal` true on the very next tick. `_flee_objective` is a pure function of current threat/creature position with no obstacle awareness, so as long as the threat stayed visible (`_flee_has_visible_threat` — true for the whole flee episode, not just the blocked moment) it recomputed the *identical* wall-blocked bearing every time. The result was an exact sawtooth: `live` (blocked 3 ticks) → SEEK seeds `explore` → next tick's sync immediately re-derives `live` from the same threat, clobbering the seed before it ever produced a waypoint → blocked again 3 ticks later → repeat, forever. The creature never actually walked an explore waypoint away from the wall.

A secondary latent issue found during the fix: `_sync_step_objective`'s goal-kind-switch reset block (top of the function) cleared `step_goal`/`explore_dir`/etc. on a fresh `goal_kind` but never reset `step_source` — a leftover `step_source == "explore"` from whatever goal the creature had *previously* been pursuing could, combined with the fix below, wrongly make a fresh entry into `GK_AVOID_HOSTILES` (with a real, currently-reachable threat) skip the live derive and mint an explore waypoint instead on its very first tick.

### Fix

Two changes, both in `motor_planner.gd`:

1. **`_sync_step_objective`'s goal-kind-switch reset** now also resets `state["step_source"] = &"live"` (matching `new_state()`'s own default), closing the leftover-`step_source` edge case above before it could interact with change 2.
2. **`GK_AVOID_HOSTILES`'s branch** now mirrors the other goal kinds' explore-latch handling instead of unconditionally re-deriving the primary (live flee) objective:
   - `refresh_targets` (a reconsideration cycle), **or** (`not has_step_goal` **and** `step_source != "explore"`) → derive live flee as before (unchanged logic/comments).
   - else `not has_step_goal` (i.e. `step_source == "explore"` and nothing minted yet — the tick right after a SEEK seed, or after `_maintain_explore_latch` clears an arrived/passed/dead-end explore waypoint) → `_mint_explore_objective_for_goal`, same call every other goal kind's fallback uses.
   - else `step_source == "explore"` (steady state, already holding a latched explore waypoint) → `_maintain_explore_latch`, same call every other goal kind's elif uses.

   The live threat re-derive is deliberately still gated behind `refresh_targets` while an explore escape is active — the same periodic-retry cadence `GK_SHELTER`/`GOAL_REST`/`GK_FIND_FOOD` already use to give their own primary source another chance without re-checking it every single tick (which is exactly what caused the sawtooth in the first place).

Deliberately **not** done: no bespoke threat-aware deflection/detour system for the sub-acute (`ff=0`) hub-driven avoid-hostiles path (unlike the acute `ff=1` Flight fast path's `_mint_flee_waypoint`, which already has its own obstacle-aware candidate-bearing scoring). This reuses the existing generic explore latch only, per explicit scope decision — minimal, consistent with the pattern every other goal kind already follows.

**Acute path unaffected:** `select_action` returns via `_select_flight_action` (which calls `_maintain_flee_latch`/`_mint_flee_waypoint`) before `_sync_step_objective` is ever reached whenever `flight_fast_path_active` is true — this fix only touches the sub-acute, hub-driven `GK_AVOID_HOSTILES` path (`ff=0`), never the acute Flight fast path.

### Regression test

`_test_motor_planner_avoid_hostiles_blocked_seek_holds_explore_no_sawtooth` (`tests/run_all.gd`): directly seeds the post-SEEK state (`_seed_explore_after_seek`) the same way `_test_motor_planner_explore_seek_seeds_waypoint` does for `GK_FIND_FOOD`, then drives 6 ticks of `_sync_step_objective` with `refresh_step_objective` held false (no reconsideration cycle) and asserts `step_source` stays `"explore"` on every one of the 6 ticks (rather than reverting to `"live"` within 1-2 ticks) and that a real non-zero escape waypoint actually got minted.

**Validity-checked:** reverted the `GK_AVOID_HOSTILES` branch to its pre-fix form (keeping the goal-kind-switch reset) — the new test failed as expected (`held 0/6`, i.e. `step_source` fell straight back to `"live"` on the very first post-seed tick). Restored the fix — full suite green again.

### Verification

Full headless suite (`godot --headless -s res://tests/run_all.gd`), 2 runs (pre- and post-restore): only the pre-existing `_test_creature_motor_stack_blocked_memory_writes` assertion failure and the two known SCRIPT ERRORs (`Nonexistent 'float' constructor`, `Nonexistent function 'set_beliefs_for_test'`) — no new failures beyond that known baseline.
...
| 2026-09-24 | **[C1](#c1--pursuit-contact-geometry-stall-fox) reopened:** live "both stuck" repro (`.smoke_logs/smoke_both_stuck.log`) confirmed the same escalate-then-give-up pinch-stall recurring, plus a new finding — decision 42's per-observation reset of `passibility_fail_count` for moving targets means a continuously-visible pinned prey never crosses `_food_pursuit_exclusions`' switch threshold, so the wolf never abandons the stuck approach. Confirmed `blk`/`cblk` are real post-hoc physics contact (`LocomotionExecutor._displace_along_facing` → `move_and_slide`/ghost-layer stop), not look-ahead prediction. Shipped a reach-scored straight-line-to-prey candidate in `_remint_alternate_pursuit_detour`, reusing flee's existing `_flee_candidate_probe`/`_apply_route_plausibility_scan` primitive, instead of always blind-rotating ±60° off the pursuer's own blocked bearing — exploits a cornered prey's actual position rather than only routing around the wall as an obstacle. Headless: pre/post-change full-suite runs (isolated via targeted `Edit` revert, not `git stash`) produced the identical single pre-existing failure (`_test_creature_motor_stack_blocked_memory_writes`) — no regression. Status `in_progress`: dedicated regression coverage for the fix (existing `_test_repro_rabbit_cornered_north_wall_live_pursuit` fixture has no closing-distance assertion) and duel re-verification still needed before re-closing. |
| 2026-09-24 | **[C1](#c1--pursuit-contact-geometry-stall-fox) closed — regression coverage added (`test-harness`):** `_test_motor_planner_pursuit_detour_prefers_straight_line_when_it_reaches_farther` (`tests/run_all.gd`) directly exercises `_remint_alternate_pursuit_detour` against a custom two-island baked navmesh where the rotated ±60° candidate is unreachable and the straight-to-prey candidate is fully open, asserting the straight candidate wins. Validity-checked red/green via a throwaway scratch script reconstructing the pre-fix rotated-only shape against the identical fixture (no edits to `motor_planner.gd`, out of scope for this task). Full headless suite: identical single pre-existing failure, zero new failures; `check_gdscript_no_tabs.py` clean. Status → `done`; maintainer also reported multiple successful live duel smoke-test captures in corner scenarios post-fix. |
...
| 2026-09-16 | **[C20](#c20-live-vs-locale-food-handoff-had-no-distance-awareness-rabbit-stuck-right-outside-a-shrub-cluster) logged and fixed** (mislabeled "C11" in this line originally — C11 was already in use for the goal-hub incumbent flip-flop above; corrected, see C20 section for the full write-up including the same-day `change_stability` follow-up)**: live-vs-locale food handoff had no distance awareness, abandoning a nearly-reached live target for a far-away locale memory.** User reported a rabbit looking "stuck right outside the circle of shrubs" during multi-predator live testing ([CM_V3_MULTI_MOBS.md](CM_V3_MULTI_MOBS.md)); traced via `motor_explore_tick.log` to `_live_vs_locale_handoff_prefers_live` (Pass 4, C2 above) — it compared candidates by pure calories-per-EAT with zero distance/travel-cost term, so a live shrub 6.4 units away (about to be reached) lost to a remembered target 27 units away because the remembered kind's estimated yield was marginally higher; the creature then reversed ~24 ticks later, net *further* from the shrub than when it started. Redesigned with the user per explicit rules, confirmed in discussion before coding: **default to live** (locale is, by construction, never something currently observed — a visible match would already be the live candidate), overridden only by two survival/economics exceptions, both intentionally *not* personality-weighted: (A) **net loss** — live's own travel cost (new `_travel_calorie_cost`, the real V3 flat-`move_calorie_per_sec`-while-moving model, not the dead-under-V3 `calorie_cost_per_unit_moved`) exceeds its yield; (B) **starvation risk** — live's full round trip (there, eat, back — modeled as 2× one-way cost, the only concretely computable proxy for "resume searching" without knowing where that search goes next) would leave calories at/below a safety margin (new `food_handoff_starvation_margin_frac`, default 0.125× `caloric_needs` — a buffer, not exactly 0, since travel cost here is a straight-line/constant-speed estimate) while locale's one-way trip would not. Discussed and deliberately deferred: biasing the *default* (not the survival exceptions) by `change_stability` (established-grounds vs. frontier lean) and jittering yield estimates by `stat_observation` (confidence) — real ideas, but new plumbing or their own right, tracked as a follow-up rather than folded into this fix. 5 handoff tests green (2 pre-existing behavior-preserved + 3 new: close-live-beats-richer-far-locale regression for the exact reported bug, net-loss exception, starvation-risk exception), full headless suite unchanged (only the pre-existing non-deterministic ObjectDB flake), live smoke-tested 3600 ticks clean. |
| 2026-09-16 | **[C21](#c21-live-food-pursuit-permanently-stuck-against-a-two-boulder-pinch-awareness-cone-dropout-starved-the-pursuit-detour-latch) logged and fixed: live food pursuit permanently stuck against a two-boulder pinch.** User reported a rabbit "stuck between two boulders" during duel playtesting; traced via `motor_explore_tick.log` (`t=915-1114`, the entire ~18s round) to a coupling gap between two previously-independent mechanisms — a single-tick awareness/LOS-cone dropout on the live food target (during a blocked, mid-turn approach) handed the objective to generic explore fallback, which stamps `step_source = "explore"`; the C1 pursuit-detour latch (`_maybe_mint_pursuit_detour_latch`) only arms while `step_source == "live"`, so the block that caused the dropout was never remembered, and the very next tick's live-food maintenance branch reapplied the same raw, still-blocked position — an infinite cycle neither mechanism's own fix (C1's sticky detour, the 2026-09-05 boxed-by-boulders-and-shrub fix) had covered. Fixed with a new bounded grace window (`live_food_awareness_grace_ticks`, default 2) in `_derive_find_food_step_objective`: a live pursuit coasts on its held objective (untouched `step_goal`, `step_source` stays `"live"`) through a couple of empty-scan ticks instead of discarding it, refilled whenever food is actually seen, and still falls through to the real fallback once exhausted so a genuine loss isn't masked. New test `_test_motor_planner_live_food_awareness_grace_holds_through_transient_dropout` (hold-through-dropout + bounded-fallback-on-exhaustion). Full headless suite: 3 runs, byte-identical to the unmodified baseline (only the pre-existing `slot >= slot_max` ObjectDB flake), no new failures. |
| 2026-09-16 | **[C22](#c22-resting-creature-never-forms-a-shelter-belief-rest-missing-from-the-opportunistic-observation-gate) logged and fixed: a resting creature never forms a shelter belief.** User watched a rabbit resting at the edge of a shrub ring flee into the open rather than toward the ring when two foxes approached, and asked whether shelter had ever been "minted." Traced to `_maybe_observe_shelter_opportunistically`'s call-site gate (`creature_motor_stack.gd`), which only fired on `EAT`/`STAY`/`WAIT` — `MotorAction.REST` was split out of generic `STAY` on 2026-08-26 (this session's C19), about two weeks before this opportunistic-observation gate was written (2026-09-11 design review), and the gate was never updated for it. A resting creature — the textbook "lingering somewhere for its own reasons" case this function's own doc comment describes — silently never got probed for shelter quality, so `shelter_map_confidence` stayed 0 no matter how long it rested there. Fixed by adding `MotorAction.REST` to the existing action check; considered making the check unconditional for all actions instead but rejected — the enclosure probe costs 8 raycasts per call, and `EAT`/`STAY`/`WAIT`/`REST` are exactly the actions where the creature holds still, keeping the check occasional rather than running on nearly every tick during ordinary locomotion. New test `_test_creature_motor_stack_rest_triggers_opportunistic_shelter_observation`: drives a real tick loop to a natural `REST`, builds an enclosing ring around wherever it actually settled, and confirms `shelter_confidence_score` goes nonzero. Separately noted but *not* fixed here (deferred, tracked as a follow-up): `GOAL_SHELTER`'s weight is flat and doesn't scale with threat proximity the way flight's `urgency_flight` does, so flight would likely still outrun even a confirmed shelter belief once real threats are closing — a design gap, not a bug. Full headless suite: 3 runs, only the pre-existing `slot >= slot_max` ObjectDB flake, no new failures. |
| 2026-09-23 | **[C24](#c24-gk_avoid_hostiles-ignored-its-own-explore-fallback-seed-and-sawtoothed-forever-east-wall-spinning) logged and fixed: `GK_AVOID_HOSTILES` ignored its own explore-fallback seed and sawtoothed forever (east-wall "spinning").** User reported a fleeing creature "spinning" near the east wall instead of escaping. Traced to `_sync_step_objective`'s `GK_AVOID_HOSTILES` arm never checking `step_source == "explore"` the way `GK_FIND_FOOD`/`GK_SHELTER`/`GOAL_REST`/the default case all already did — `apply_blocked_objective_resolution`'s §9 SEEK fallback seeds an explore escape after 3 consecutive blocked ticks (`_seed_explore_after_seek`, clears `step_goal_set`, latches `step_source = "explore"`), but the very next tick's `not has_step_goal` unconditionally re-derived `_flee_objective()` — obstacle-unaware and pure, so a still-visible threat reproduced the identical wall-blocked bearing every time — clobbering the seed before it ever minted a waypoint (`live → blocked → explore-seeded → clobbered → live → ...`, forever). Fixed by mirroring the other goal kinds' explore-latch handling: while `step_source == "explore"`, mint/maintain the explore waypoint (`_mint_explore_objective_for_goal` / `_maintain_explore_latch`) instead of re-deriving live flee, only re-checking the live threat on the next reconsideration cycle (`refresh_targets`) — same cadence the other goal kinds already use. Also closed a related edge case found while implementing: the goal-kind-switch reset block never reset `state["step_source"]`, so a leftover `"explore"` from a *previous* goal could wrongly suppress the live-flee derive on `GK_AVOID_HOSTILES`'s first tick; now reset to `"live"` alongside the block's other per-goal field resets. Acute Flight fast path (`ff=1`, `_mint_flee_waypoint`/`_maintain_flee_latch`) untouched — `select_action` returns via `_select_flight_action` before `_sync_step_objective` is ever reached while `flight_fast_path_active`. New test `_test_motor_planner_avoid_hostiles_blocked_seek_holds_explore_no_sawtooth`: seeds the post-SEEK state directly (mirroring `_test_motor_planner_explore_seek_seeds_waypoint`'s pattern for `GK_FIND_FOOD`) and asserts `step_source` holds `"explore"` across 6 ticks with a real waypoint minted, instead of reverting to `"live"` within 1-2 ticks. Validity-checked: reverting the fix made the new test fail (`held 0/6`); restored, full suite green. Full headless suite: 2 runs, only the pre-existing `_test_creature_motor_stack_blocked_memory_writes` assertion and the two known SCRIPT ERRORs, no new failures. |
| 2026-09-24 | **[C23](#c23--stuck-target-detectionexclusion-review-decision-21s-mint-site-audit-finally-closed-two-mechanisms-found-not-to-be-duplicates-after-all) `<<Comment>>`s resolved (doc-only):** the user approved amending [PHYSICS_SQUEEZE.md](PHYSICS_SQUEEZE.md) decision 21 to match the code. The plain `is_waypoint_dead_end` check C23 wired is the intended design; the general belief-overlap override is rejected (reasons recorded there). The only rescue is shelter-only, for locale-cell and choke flee destinations (decision 45 B). C23's conclusions stand unqualified. |
