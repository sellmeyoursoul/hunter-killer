...
| **2. Mob pursuit** | Duel carnivore (fox pack on Mob), prey **in live cone** (`predator_prey_awareness_cone_extra` 0, no boot lock) | No straight-line rush at round start; patrol/search until prey enters awareness, then chase. Memory expand only after first live sight. **Chaos:** high when no prey in cone (`motor_no_goal_chaos_mul`); low when chasing (`motor_goal_sight_chaos_mul`). |
| **2b. Run variance** | Replay duel with ship profile; fox patrol before contact | Patrol headings differ between runs (tie shuffle + no-goal chaos + plateau random on equal costs). Chase leg stays directed once prey in cone. |
