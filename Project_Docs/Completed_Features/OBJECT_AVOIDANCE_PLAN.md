...
**Parent design:** [ENVIRONMENT_MODEL_PLAN.md](../Definitive_Features/ENVIRONMENT_MODEL_PLAN.md) — this document narrows scope to **2D** locomotion: squeeze (**Mode A**, **`passible == false`**), shrubs / brush (**Mode B**, **`passible == true`** + **`movement_impact`** + **`fit_size`**), mob detours, and **`movement_speed_multiplier(creature_size, env)`**. It does **not** supersede the parent catalog; it pins semantics left open there.

**Post-archive carry-forward:** When this plan moves to **Completed_Features**, keep **deferred** policy and `<<Comment>>` follow-ups alive in active docs: [ENVIRONMENT_MODEL_PLAN.md](../Definitive_Features/ENVIRONMENT_MODEL_PLAN.md) §10 (consolidated list), [ENHANCEMENT_BACKLOG_PLAN.md](../ENHANCEMENT_BACKLOG_PLAN.md) (parking-lot items), [CREATURE_EVOLUTION_AND_MOTOR_GENOME.md](../Draft_Features/CREATURE_EVOLUTION_AND_MOTOR_GENOME.md) (motivation ↔ motor), [CREATURE_MODEL_PLAN.md](../Draft_Features/CREATURE_MODEL_PLAN.md) §9 (ENGINE vs AI control parity), [CREATURE_MEMORY.md](../Draft_Features/CREATURE_MEMORY.md) (creature memory: food, hiding, LoS).
...
**Align with:**  
- [CREATURE_MODEL_PLAN.md](../Draft_Features/CREATURE_MODEL_PLAN.md) — `size` / `weight` catalog (`weight` unused here).  
...
**Per-kind memory (`terrain_kind_id`):** Separate memories for **squeeze affordance** vs **slowdown curves** may share the same ID space or sub-keys — **TBD** when implementing; the invariant is **mud ≠ deep_snow ≠ squeeze-rock-wall-kind** so lessons do not cross-contaminate ([ENVIRONMENT_MODEL_PLAN.md](../Definitive_Features/ENVIRONMENT_MODEL_PLAN.md) property catalog).

**Canonical carry-forward:** Items above and other deferred `<<Comment>>` threads are duplicated in [ENVIRONMENT_MODEL_PLAN.md](../Definitive_Features/ENVIRONMENT_MODEL_PLAN.md) §10 so they survive archiving this file.
