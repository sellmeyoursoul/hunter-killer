...
| [Draft_Features/CONVERT_TO_3D.md](Draft_Features/CONVERT_TO_3D.md) | **2D→3D migration umbrella:** 3D-only production (`main_3d`), grasslands mesh playfield (no PNG terrain), `CharacterBody3D` duel, `Vector3` motor; defers creature stack to [CREATURE_3D_ARCHITECTURE.md](Draft_Features/CREATURE_3D_ARCHITECTURE.md). |
