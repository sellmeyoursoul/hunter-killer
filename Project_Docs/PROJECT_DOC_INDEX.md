...
| [Draft_Features/COMBAT_RESOLVED.md](Draft_Features/COMBAT_RESOLVED.md) | **Melee combat contract:** contact detection, damage vs. point pools (`stat_to_point`), `tactic_fight_active` motor flag, salient write on outcome; **implementation gated on [CREATURE_MOVEMENT_V3](Draft_Features/CREATURE_MOVEMENT_V3) shipping**; also depends on **[SHARED_STATTOPOINT_PLAN.md](Draft_Features/SHARED_STATTOPOINT_PLAN.md)**. Split three ways per [project-docs.mdc](../.cursor/rules/project-docs.mdc) — companions: [COMBAT_IMPLEMENTATION.md](Draft_Features/COMBAT_IMPLEMENTATION.md) (build plan/status), [COMBAT_HISTORY.md](Draft_Features/COMBAT_HISTORY.md) (decision rationale). |
...
| [Draft_Features/CREATURE_MOVEMENT_V2.md](Draft_Features/CREATURE_MOVEMENT_V2.md) | **Unified motor refactor (phase source of truth):** `creature_motor`, **`SeekCandidate`** ingress, **`MotorContext`**; Phase **4.5** POST_LOS planner → **[POST_LOS_MOVEMENT.md](Draft_Features/POST_LOS_MOVEMENT.md)**. |
| [Draft_Features/POST_LOS_MOVEMENT.md](Draft_Features/POST_LOS_MOVEMENT.md) | **POST_LOS navigation planner (Phase 4.5):** goal table, seek cycle, Observation replan **n**, multistep/explore/backtrack; parent roadmap **[CREATURE_MOVEMENT_V2.md](Draft_Features/CREATURE_MOVEMENT_V2.md)**. |
